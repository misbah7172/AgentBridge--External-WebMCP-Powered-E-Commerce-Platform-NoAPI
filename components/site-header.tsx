...
export async function SiteHeader() {
  ...
  return (
    <header data-agentbridge-session={user ? "authenticated" : "anonymous"} className="border-b border-slate-200 bg-white">
      <div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between gap-6 px-5">
        ...
        <nav className="flex items-center gap-5 text-sm font-medium text-slate-600">
          {links.map(([label, href]) => <Link key={href} data-agentbridge-nav={href.slice(1)} className="hover:text-ink" href={href}>{label}</Link>)}
